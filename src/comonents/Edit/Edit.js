     ...
     <div className='w-[60vw] bg-rose-700 border-2 border-rose-900 rounded-3xl'>
      ...
      <form className="flex flex-col items-center  w-full" onSubmit={SubmitHandler}>
        <input type="text"
         ...
         name="singer" id="album-singer" placeholder="Singer/Band" defaultValue={current.singer} />
        <input type="text"
                  className='block w-5/6 max- my-1 mx-3 rounded h-10 border-[1px] text-center border-rose-300'
         name="album" id="album-album" placeholder="Album" defaultValue={current.album}  />
        <input type="text"
                  className='block w-5/6 max- my-1 mx-3 rounded h-10 border-[1px] text-center border-rose-300'
         name="imageUrl" id="album-img" placeholder="Image url" defaultValue={current.imageUrl}   />
        <input type="text"
                  className='block w-5/6 max- my-1 mx-3 rounded h-10 border-[1px] text-center border-rose-300'
         name="release" id="album-release" placeholder="Release date" defaultValue={current.release}   />
        <input type="text"
                  className='block w-5/6 max- my-1 mx-3 rounded h-10 border-[1px] text-center border-rose-300'
         name="label" id="album-label" placeholder="Label" defaultValue={current.label}    />
        <input type="text"
                  className='block w-5/6 max- my-1 mx-3 rounded h-10 border-[1px] text-center border-rose-300'
         name="sales" id="album-sales" placeholder="Sales" defaultValue={current.sales} />
        <button type="submit"

...
import { AuthContext } from "../../context/AuthContext"
 ...
 const Details =()=>{
  ...
  const current=data.find(x=>x._id===id)
    ...
    return   <section id="details " className='bg-white min-h-[80vh] flex flex-col items-center'>
    <h1 className="text-bold text-2xl uppercase text-center py-10 italic font-mono">Album Details</h1>
     <div className=' w-fit group flex lg:gap-9 gap-3 transition-opacity p-10 flex-col md:flex-row md:justify-center items-center'>
    <img src={current.imageUrl} className='w-72 md:w-[40vh] lg:w-[50vh] group  group-hover:rotate-1 group-hover:scale-105 transition-transform shadow-2xl relative z-10' alt='album img' />
